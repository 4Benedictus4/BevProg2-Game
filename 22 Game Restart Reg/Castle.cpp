...
void Castle::Log(){
    Building::Log();
    std::string d, a, h, dm, am, r, ap, sp;
    std::stringstream ss;
    int Ap, Sp;
    if(atkpoints<0) Ap=0;
    else Ap=atkpoints;
    if(spawnpoints<0) Sp=0;
    else Sp=spawnpoints;
    ss<<def<<" "<<atk<<" "<<hp<<" "<<def_mod<<" "<<atk_mod<<" "<<range<<" "<<Sp<<" "<<Ap;
    ss>>d>>a>>h>>dm>>am>>r>>sp>>ap;
    gout<<color(white)<<move_to(420,YY-85)<<text("SP: ")<<move_to(500,YY-85)<<text("AP: ")<<move_to(460,YY-85)<<text(sp)<<move_to(540,YY-85)<<text(ap)<<move_to(545,YY-65)<<text(a)<<text(" + ")<<text(am)<<move_to(545,YY-45)<<text(d)<<text(" + ")<<text(dm)<<move_to(545, YY-25)<<text(h)<<move_to(545, YY-5)<<text(r);
}
