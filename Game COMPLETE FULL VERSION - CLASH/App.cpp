...
void App::Reg(Player* P){
    PlX.push_back(P);
}

void App::Reg(Plain* P){
    PX.push_back(P);
    LX.push_back(P);
    TX.push_back(P);
}

void App::Reg(Forest* F){
    FX.push_back(F);
    LX.push_back(F);
    TX.push_back(F);
}
...
void App::Remove(Building* erase_b, bool delCastle){
    ...
    if(delCastle) cX.erase(std::remove(cX.begin(), cX.end(), erase_b), cX.end());
}

void App::Remove(Unit* erase_u){
...
void App::Remove(Player* erase_p){
    PlX.erase(std::remove(PlX.begin(), PlX.end(), erase_p), PlX.end());
}

App::App(){
    int PC;
    std::cout<<"Number of players (1-4)?"<<std::endl;
    std::cin>>PC;
    int p=0, f=0, h=0;
...
void App::Run(App &A){
    ...
    while(gin >> ev && ev.keycode!=key_escape){
        ...
        if(ev.type==ev_timer){
            ...
            bool delCastle=false;
            for(Castle* c : cX){
                if(c==erase_b){
                    delCastle=true;
                    break;
                }
                else delCastle=false;
            }
            Remove(erase_b, delCastle);
            delete erase_b;
            for(Unit* u : UX){                              /// DEATH
                if(u->get_stats()["hp"]<=0){
                    for(Landscape* l : LX){
