...
/// ATTACK OR GARRISON ON RIGHT-CLICK --- SOLVED BY PLAYERS / TURNS
                        ...
                        if(c2->OnMouse(posX, posY) && c!=c2){
                            ...
                            if(abs(c2x-cx)<=LandscapeSize*c->get_stats()["r"] && abs(c2y-cy)<=LandscapeSize*c->get_stats()["r"]){
                                ...
                                for(Building* b : BX){
                                    ...
                                    else if(c2->get_pos()["x"]==b->get_pos()["x"] && c2->get_pos()["y"]==b->get_pos()["y"] && c2==b){
                                        b->take_damage(c->get_stats()["atk"]);
                                        if(c->get_stats()["r"]<=1){
                                            c->take_damage((b->get_stats()["atk"])-3);
                                        }
                                        for (Unit* u : UX){
