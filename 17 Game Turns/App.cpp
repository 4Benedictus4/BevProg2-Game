...
App::App(){
    ...
    BX.push_back(C4);
    cX.push_back(C4);
    Warrior* W1=new Warrior(200+Usize_diff, 200+Usize_diff, UnitSize, UnitSize, 1, P1, "w");
    TX.push_back(W1);
    CX.push_back(W1);
    UX.push_back(W1);
}
                        ...
                        if(c2->OnMouse(posX, posY)){
                            ...
                            if(c->Owner()!=c2->Owner()){
                                if(abs(c2x-cx)<=LandscapeSize*c->get_stats()["r"] && abs(c2y-cy)<=LandscapeSize*c->get_stats()["r"] && c->CanAtk()){
                                    ...
                                    if(c2!=Garr){                                      ///deleted: !isonB &&  -  SOLVED last castle's invincibility problem  -  INVESTIGATE for cause & further bugs!
                            ...
                            if(c->Owner()==c2->Owner()){                                            /// POSITION SWAPPING
                                cx=c->get_pos()["x"];
                                cy=c->get_pos()["y"];
                                for(Landscape* lprev : LX){
